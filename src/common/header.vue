    ...
    <header class="w">
      <div class="w-box">
        ...
        <div class="right-box">
          ...
          <div class="nav-list">
          <!-- 加上native限制符，因为element-ui把input进行了封装，原事件就不起作用了 -->
            <el-autocomplete placeholder="请输入商品信息" icon="search" v-model="input" minlength=1 maxlength=100 :fetch-suggestions="querySearchAsync"
             @select="handleSelect" :on-icon-click="handleIconClick" @keydown.enter.native="handleIconClick">
            </el-autocomplete>
...
export default {
  ...
  computed: {
    // 加入购物车列表 是否登录 是否进入入购物车 是否显示购物车 用户信息
  ...
  methods: {
    ...mapMutations(['INIT_BUYCART', 'ADD_ANIMATION', 'SHOW_CART', 'RECORD_USERINFO']),
    toCart () {
    ...
    handleSelect (item) {   // 选中的文字
      this.input = item.value
    },
    changePage (v) {     // 导航栏中文字样式改变
      this.choosePage = v
    },
    changGoods (v) {
      this.changePage(v)
